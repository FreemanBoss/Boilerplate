...
class TestLoginRoute:
    ...
    @pytest.mark.asyncio
    async def test_login_no_email(self, client, mock_johnson_user_dict, test_get_session, test_setup):
    ...
    @pytest.mark.asyncio
    async def test_login_superadmin_fake_email_domain(self, client, mock_johnson_user_dict, test_setup):
        ...
        login_response = client.post(
            ...
            json={"email": "johnson@test.com", "password": "Johnson1234@"},
        )
    ...
    @pytest.mark.asyncio
    async def test_login_success(self, client, test_get_session, mock_johnson_user_dict, test_setup):
        ...
        stmt = user_roles.insert().values(user_id=new_user.id, role_id=role.id)
        await test_get_session.execute(stmt)
        await test_get_session.commit()

        login_response = client.post(
            url="/api/v1/auth/login",
            json={"email": "johnson@gmail.com", "password": "Johnson1234@"},
        )
    ...
    @pytest.mark.asyncio
    async def test_login_email_not_verified(self, client, test_get_session, mock_johnson_user_dict, test_setup):
        ...
        data = login_response.json()
        assert data["message"] == "Email not verified, check your inbox or spam. If link has expired, request for another link with the email used for registration"
    ...
    @pytest.mark.asyncio
    async def test_login_unsuccess_not_exist_email(self, client, mock_johnson_user_dict, test_setup):
    ...
    @pytest.mark.asyncio
    async def test_login_unsuccess_wrong_password(self, client, mock_johnson_user_dict, test_setup):
    ...
    @pytest.mark.asyncio
    async def test_login_user_is_not_active(self, client, test_get_session, mock_johnson_user_dict, test_setup):
        ...
        login_response = client.post(
            url="/api/v1/auth/login", json={"email": "johnson@gmail.com", "password": "Johnson1234@"}
        )
    ...
    @pytest.mark.asyncio
    async def test_login_user_is_suspended(self, client, test_get_session, mock_johnson_user_dict, test_setup):
    ...
    @pytest.mark.asyncio
    async def test_login_user_is_deleted(self, client, test_get_session, mock_johnson_user_dict, test_setup):
